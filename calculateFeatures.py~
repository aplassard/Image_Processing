...
def calculatefeatures(dictionary,left=None,right=None,top=None,bottom=None):
	...
	H = flattenHaralick(H)
	vector.extend(H)
	H=HaralickFeatures(dictionary[RGB][bottom:top,left:right,RED])
	H = flattenHaralick(H)
	vector.extend(H)
	H=HaralickFeatures(dictionary[RGB][bottom:top,left:right,BLUE])
	H = flattenHaralick(H)
	vector.extend(H)
	del H
	T=tasfeatures(dictionary[grayscale][bottom:top,left:right])
	vector.extend(T)
	T=tasfeatures(dictionary[RGB][bottom:top,left:right,GREEN])
	vector.extend(T)
	T=tasfeatures(dictionary[RGB][bottom:top,left:right,RED])
	vector.extend(T)
